    ...
    for pb in sorted_bones:
        if pb.name in final_globals:
            ...
            if pb.parent:
                ...
                # Exact formulation of Blender's pose system inverse math
                rest_rel_inv = pb.bone.matrix_local.inverted() @ pb.parent.bone.matrix_local
...
# ═══════════════════════════════════════════════════════════════════════════════
# Operator: Manual mirror (secondary)
# ═══════════════════════════════════════════════════════════════════════════════
...
class UE_PT_MirrorPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        op.axis          = props.axis
        op.mirror_center = props.mirror_center
...
class UE_MirrorProperties(bpy.types.PropertyGroup):
    axis: bpy.props.EnumProperty(name="Axis", default="X", items=[("X", "X", ""), ("Y", "Y", ""), ("Z", "Z", "")])
    mirror_center: bpy.props.BoolProperty(name="Mirror Center Bones", default=False)
...
_CLASSES = [UE_MirrorProperties, UE_OT_SmartMirror, UE_OT_MirrorPose, UE_PT_MirrorPanel]
