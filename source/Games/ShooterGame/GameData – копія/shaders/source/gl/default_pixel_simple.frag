...
float computePCSS(
    ...
    for (int xo = -br; xo <= br; ++xo) {
        for (int yo = -br; yo <= br; ++yo) {
            vec2 uv = clamp(
                pc.xy + vec2(xo, yo) * texelSize * length(vec2(xo, yo)) * 3.0,
                tileMin, tileMax
            );
...
vec2 GetDirectionalShadow(
    ...
    float sampleScale = mix(4.0f, 1.0f, directNoDetail);
    int ir = int(ceil(float(pcfRadius)));
    ...
    indirect = mix(indirect, 1.0f, 0.5f);
    ...
    float finalIndirect = mix(indirect * 0.5 + 0.5, 1.0, directNoDetail);
