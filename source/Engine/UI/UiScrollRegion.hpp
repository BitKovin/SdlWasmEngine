...
public:
    ...
    // ── Draw ──────────────────────────────────────────────────────────────────
    // The content pane is clipped by a stencil mask.  We pass finalizedMatrix
    // to PushMask so the mask quad lives in the same transformed space as the
    // content quads — rotated scroll regions clip correctly.
    void Draw() override
    {
        ...
        {
            // Build a matrix that covers only the content area (excludes scrollbar).
            // This is the element's own matrix with no extra adjustment for X,
            // because the scrollbar sits at the right edge; the mask width is just
            // reduced.
