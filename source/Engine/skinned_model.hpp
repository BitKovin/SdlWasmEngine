...
#include "Logger.hpp" // for Logger::Error in AdoptVisualTierGeometry's bone-ID remap
...
namespace roj
{
    ...
    struct SkinnedModel
    {
        ...
        {
            // The Visual tier's assimp reparse assigns its own bone IDs into
            // parsed.boneInfoMap - independently of, and not necessarily in the
            // same order as, the IDs the Logic tier already committed to this
            // model's live skeleton (boneInfoMap below). They only agree by
            // accident: the fast glTF path assigns IDs in skin.joints[] order,
            // while assimp assigns them in whatever order it encounters
            // aiMesh::mBones while walking this reparse's own mesh list - a
            // different algorithm that happens to produce the same order only
            // for the simplest files. Since mesh vertex BlendIndices are
            // written using parsed's numbering, they have to be translated to
            // the live skeleton's numbering (by bone name) before these meshes
            // are usable - otherwise skinning silently applies the wrong
            // bone's matrix to the wrong vertices: the skeleton, evaluated on
            // its own, looks completely correct, but every skinned mesh looks
            // broken. This remap is what actually closes that gap - it isn't
            // just a sanity check, every Visual-tier adoption goes through it.
            if (parsed.boneCount > 0)
            {
                std::vector<hashed_string> parsedIdToName(parsed.boneCount);
                for (auto& [name, info] : parsed.boneInfoMap)
                    if (info.id >= 0 && info.id < parsed.boneCount)
                        parsedIdToName[info.id] = name;

                std::vector<int> remap(parsed.boneCount, -1);
                bool needsRemap = false;
                bool missingBone = false;
                for (int id = 0; id < parsed.boneCount; ++id)
                {
                    auto it = boneInfoMap.find(parsedIdToName[id]);
                    if (it != boneInfoMap.end())
                    {
                        remap[id] = it->second.id;
                        if (remap[id] != id) needsRemap = true;
                    }
                    else
                    {
                        missingBone = true; // mesh uses a bone the live skeleton doesn't have at all
                    }
                }

                if (missingBone)
                    Logger::Error("SkinnedModel: Visual tier mesh references a bone missing from the Logic tier skeleton - skinning will be wrong for those vertices.");
                ...
                if (needsRemap)
                {
                    for (auto& mesh : parsed.meshes)
                    {
                        ...
                        {
                            float* w = glm::value_ptr(v.BlendWeights);
                            for (int k = 0; k < MAX_BONE_INFLUENCE; ++k)
                            {
                                if (w[k] == 0.f) continue;
                                const int oldId = v.BlendIndices[k];
                                if (oldId >= 0 && oldId < (int)remap.size() && remap[oldId] >= 0)
                                    v.BlendIndices[k] = remap[oldId];
                                // else: already logged above via missingBone -
                                // leave this slot as-is rather than guessing.
                            }
                        }
                    }
                }
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    // Parallel animation-frame baking for UsePrecomputedFrames mode.
    //
    // Baking (see EvaluatableClip::bakedFrames) means running a full animation
    // simulation per clip - real CPU cost that scales with clip count and
    // duration. It only needs skeleton + clip track data, nothing mesh-related,
    // so it doesn't belong on the Logic tier's synchronous commit path (that
    // path is supposed to stay cheap - see AssetRegistry::LoadLogicTierNow),
    // and it doesn't need to wait for the Visual tier's mesh reparse either -
    // both tiers' clips can be baked the same way, as soon as skeleton + clips
    // exist, regardless of which parser produced them.
    //
    // The two-step split below exists for one reason: avoid mutating a live
    // clip's bakedFrames from a background thread while something else might be
    // reading it (e.g. this same cached model, already in use elsewhere, being
    // reloaded). BakeAllClipsParallel() only ever produces new data - it never
    // writes to the model - so it's safe to call from any thread, with each
    // clip baked concurrently. ApplyBakedAnimationFrames() is where that data
    // actually becomes visible: call it from wherever GPU buffer uploads happen
    // for this model (the main thread), so the write lands on the same thread
    // that reads bakedFrames during animation evaluation for rendering, instead
    // of racing it from the Loader thread.
    // ─────────────────────────────────────────────────────────────────────────────

    struct BakedClipsResult
    {
        std::unordered_map<hashed_string, std::vector<EvaluatableClip::BakedFrame>> frames;
        float frameInterval = 0.f;
    };

    // Safe to call from any thread, including in parallel with other work on
    // the same model - reads model.skeleton/model.clips but never writes to
    // model. Each clip is baked independently and concurrently; the frameRate
    // default matches BAKED_FRAME_RATE in skinned_model.cpp (kept as a literal
    // here rather than a shared constant to avoid a cross-TU dependency for one
    // number - change both if you change the bake rate).
    BakedClipsResult BakeAllClipsParallel(const SkinnedModel& model, float frameRate = 30.f);

    // Main-thread only (or whatever thread reads EvaluatableClip::bakedFrames
    // during animation evaluation) - see the banner comment above for why.
    // Splices baked frames onto the live model's existing EvaluatableClip
    // objects by name; a clip present in `baked` but no longer in model.clips
    // (e.g. UnloadAll() ran first) is silently skipped.
