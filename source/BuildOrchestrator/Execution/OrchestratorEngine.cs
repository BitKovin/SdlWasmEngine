...
public sealed class OrchestratorEngine
{
    ...
    private readonly bool _forceGameData;
    private readonly string _repoRootFull;
    private readonly string _logRootFull;

    public OrchestratorEngine(BuildConfig config, bool dryRun, bool clean, bool forceGameData)
    {
        _config = config;
        _dryRun = dryRun;
        _forceGameData = forceGameData;
    ...
    public async Task<List<TargetResult>> RunAsync(List<BuildTarget> targets, CancellationToken cancellationToken)
    {
        ...
        if (_config.FinalizeGameDataOnce)
        {
            var gameDataResult = await GameDataFinalizer.RunAsync(
                _config, _repoRootFull, _logRootFull, _dryRun, _forceGameData, cancellationToken);

            if (!gameDataResult.Success)
            {
                // Every target depends on this having succeeded (either preloading the shared
                // GameData directly, or copying it in POST_BUILD) — no point spending 20+ minutes
                // building platforms that would all ship broken/missing game data.
                Console.WriteLine();
                Console.Error.WriteLine($"GameData finalize failed — aborting before any platform build started.");
                var abortedResults = new List<TargetResult> { gameDataResult };
                PrintSummary(abortedResults, DateTimeOffset.Now - start);
                WriteSummaryFile(abortedResults);
    ...
    private void WriteSummaryFile(List<TargetResult> results)
    {
        try
        {
            Directory.CreateDirectory(_logRootFull);
            var path = Path.Combine(_logRootFull, "summary.json");
